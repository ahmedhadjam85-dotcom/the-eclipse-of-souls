  ...
  poll(dt) {
    ...
    if (this.mode === 'xbox') {
      ...
      if (pad) {
        ...
        let tdx = Math.abs(ax) > 0.15 ? ax : 0;
        let tdy = Math.abs(ay) > 0.15 ? ay : 0;
        // D-pad (buttons 12-15) works alongside the stick — whichever
        // the player prefers. D-pad presses are digital (full strength).
