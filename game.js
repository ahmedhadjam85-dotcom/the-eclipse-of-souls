...
class InputManager {
  constructor(mode) {
    ...
    this.dx = 0; this.dy = 0;     // current movement direction, read directly by the player each frame
    ...
    // THE FIX: if the window/tab loses focus while a key is held (very
    // common on TV/console browsers — a controller-to-keyboard bridge,
    // a system overlay, alt-tabbing), the matching keyup can be missed
    // entirely. That leaves a key "stuck" pressed forever, which is
    // exactly why the character kept walking by itself. Clearing all
    // held keys the moment focus is lost guarantees nothing stays stuck.
    const releaseAll = () => {
      this._keys = {};
      this._attackHeldNow = false;
      this.dx = 0; this.dy = 0;
      if (this._touchDir) { this._touchDir.up = this._touchDir.down = this._touchDir.left = this._touchDir.right = false; }
    };
    ...
    window.addEventListener('gamepaddisconnected', () => { this._gamepadIndex = null; releaseAll(); });

    if (mode === 'phone') this._setupTouch();
  }

  _setupTouch() {
    // Simple 4-direction D-pad buttons instead of an analog stick —
    // each button is its own element, so the browser already keeps
    // their pressed states independent (no manual finger-tracking
    // needed like the old joystick required).
    this._touchDir = { up: false, down: false, left: false, right: false };

    const bindDpadButton = (id, key) => {
      const btn = document.getElementById(id);
      const press = e => { e.preventDefault(); this._touchDir[key] = true; };
      const release = () => { this._touchDir[key] = false; };
  ...
  poll(dt) {
    ...
    // phone mode: read the 4 D-pad buttons, same digital logic as keyboard
    if (this.mode === 'phone') {
      let dx = 0, dy = 0;
      if (this._touchDir.left) dx -= 1;
      if (this._touchDir.right) dx += 1;
      if (this._touchDir.up) dy -= 1;
      if (this._touchDir.down) dy += 1;
      this.dx = dx; this.dy = dy;
    }
...
class DialogueRunner {
  constructor() {
    ...
    // Space is also watched as a safety-net fallback in case a given
    // browser/controller-bridge doesn't map buttons the standard way.
    this._watchingPad = false;
    this._prevY = false;
    this._watchGamepad();
    window.addEventListener('keydown', e => {
      if (this._watchingPad && (e.code === 'Space' || e.code === 'Enter')) this._advance();
    });
  }
  ...
  _watchGamepad() {
    ...
    // Checks EVERY connected pad (not just index 0), and accepts
    // either the boolean `.pressed` flag or an analog `.value` above
    // a threshold — some browsers/controllers only populate one of
    // the two reliably.
    const loop = () => {
      if (this._watchingPad) {
        ...
        for (const pad of pads) {
          ...
          const y = pad.buttons[3];
          const a = pad.buttons[0];
          if ((y && (y.pressed || y.value > 0.5)) || (a && (a.pressed || a.value > 0.5))) {
            yPressed = true; break;
          }
        }
...
// ---------------------------------------------------------
// Per-chapter arena look — color + border style changes as
// the story escalates (warm/solid → eerie/dashed → intense/thick)
// ---------------------------------------------------------
const ArenaStyles = {
  1: { color: '#e0533b', lineWidth: 2, dash: [] },       // human duels — warm, solid
  2: { color: '#8a3bff', lineWidth: 2, dash: [7, 5] },   // monster realm — eerie, dashed
  3: { color: '#ff1b3b', lineWidth: 3, dash: [] },       // endgame — intense, thick
};
...
const Engine = {
  ...
  async start(mode) {
    this.music.stop(); // stop the menu theme — gameplay music takes over per-boss
    ...
    // Show the on-screen D-pad + attack button ONLY in phone mode.
  ...
  _render() {
    ...
    // arena box — visual style changes per chapter (see ArenaStyles)
    const style = ArenaStyles[this.currentChapter] || ArenaStyles[1];
    ctx.fillStyle = '#0d0d16';
    ctx.fillRect(a.x, a.y, a.w, a.h);
    ctx.strokeStyle = this.attackFlashTimer > 0 ? '#ffffff' : style.color;
    ctx.lineWidth = style.lineWidth;
    ctx.setLineDash(style.dash);
    ctx.strokeRect(a.x, a.y, a.w, a.h);
    ctx.setLineDash([]);

    // corner accent brackets — a small "targeting reticle" touch,
    // colored to match the chapter, cheap to draw (just 8 short lines)
